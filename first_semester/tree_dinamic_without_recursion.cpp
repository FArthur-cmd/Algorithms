...
/*
 Обход в глубину позволяет найти расстояние до вершин, а именно:
     ...
     Что касается дистанции, то она равна сумме дистанции для ребенка + number_of_points,
...
*/
void depth_first_search(int current_point,
...
/*
 ...
 Пример:
     ...
     / \   \
    D   E   F
    Запускаем для листьев, так как расстояния для родителей зависят от
    расстояния детей.
...
int main() {
    ...
    for(long long int i = distance.size()-1; i>=0; i--) {
        depth_first_search(i, tree, number_of_points, distance);
    }
    std::cout << distance[0]<<"\n";
    for (int i = 1; i < n; i++) {
